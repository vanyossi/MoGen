...
struct mgnp_features_moead_fcrbf {
    gsl_vector *sigma;
};
...
typedef struct mgn_moead_fcrbf_data mgnp_moead_fcrbf_data;
typedef struct mgnp_moead_fcrbf_mdl_p mgnp_moead_fcrbf_mdl_p; // moead model params
...
{
    return (mgnp_moead_fcrbf_data*)moa->features;
}

bool mgnp_moead_fcrbf_stop(mgnMoa *moa)
{
    mgnp_moead_fcrbf_data *adata = mgn_moead_fcrbf_features(moa);
    return (moa->tot_exec >= adata->max_eval);
}

void mgnp_moead_fcrbf_set_ga_vals(mgnMoa *moa, mgn_ga_sets *ga)
{
    UNUSED(moa);
    UNUSED(ga);
}
...
// moead de min optim
struct mgnp_moead_fcrbf_s_optim_ef_p {
    mgnp_de_ef_param_m()
    gsl_matrix *y_t; //training matrix
//    gsl_vector *y_cur;
};

struct mgnp_moead_fcrbf_s_optim_p {
...
int mgnp_moead_fcrbf_s_optim_mop_min(gsl_vector *f1
                                  ,gsl_vector *f2
                                  ,mgn_de_ef_param *ef_prm) {
    UNUSED(ef_prm);
//    struct mgnp_moead_fcrbf_s_optim_ef_p *prm = (struct mgnp_moead_fcrbf_s_optim_ef_p*)ef_prm;
//
...
// g is none
void mgnp_moead_fcrbf_s_optim_mop(gsl_vector *x
                               ,gsl_vector *f
                               ,gsl_vector *g
                               ,struct mgnp_moead_fcrbf_s_optim_p *prm
)
{
    UNUSED(g);
//    struct mgnp_moead_fcrbf_s_optim_p *prm = (struct mgnp_moead_fcrbf_s_optim_p*)m_prm;

    mgn_rbf_create_phi(prm->tset->x,prm->cluster_d,x,prm->rbf,prm->m_phi);
...
)
{
    ...
    mop->eval = mgn_cast_eval(mgnp_moead_fcrbf_s_optim_mop);
    ...
    struct mgnp_moead_fcrbf_s_optim_ef_p *ef_prm = malloc(sizeof(*ef_prm));
    ef_prm->y_t = tset->f;
...
gsl_vector *mgnp_moead_fcrbf_find_lambda(mgn_pop_matrix *tset, struct mgnp_rbf_weigts *res, size_t size)
{
...
//    mrbf->l_lhci_lim = false;
    ...
    size_t xsize = moead_fcrbf->solution->iparams.x_size;
...
void mgnp_moead_fcrbf_mdl_mop(gsl_vector *x, gsl_vector *f, gsl_vector *g, mgnp_moead_fcrbf_mdl_p* prm)
{
    UNUSED(g);
...
void mgnp_moead_fcrbf_mdl_mop_param_free(mgnp_moead_fcrbf_data *moead_fcrbf)
{
...
void mgnp_moead_fcrbf_mdl_optim(mgnp_moead_fcrbf_mdl_p *mdl_p, mgn_pop *p_e, mgn_ptr *m_wl)
{
 ...
 *
 */
typedef struct mgnp_moead_fcrbf_selhash {
    int id;
    double* key;
    UT_hash_handle hh;
} mgnp_selhash;
...
/**
void mgnp_moead_fcrbf_select_pop(mgn_pop *p_dest,mgn_pop* p_orig, gsl_vector_int* index)
{
//    // TODO change comments to DEBUG clauses
//    mgnp_selhash *hashes = NULL;
//    mgnp_selhash *sh, *cur_sh; // = malloc(sizeof(*sh));
...
**/
// TODO indv protoype
void mgnp_moead_fcrbf_update_refine(mgn_pop *pop_newt, mgn_pop *pop_sel)
{
...
void mgnp_moead_fcrbf_update(mgnp_moead_fcrbf_data *mrbf, mgn_pop *pop_sel)
{
    for (size_t i = 0; i < pop_sel->size; ++i) {
...
void mgn_moead_fcrbf_run(mgnMoa *moa)
{
...
//    }
    ...
    for (size_t i = 0; i < moead_fcrbf->mdl_size; ++i) {
...
//        puts("_-___________-");
        ...
        // DEBUG
        moead_fcrbf->mdl_rbf[i].m_phi = mgn_rbf_create_phi(moead_fcrbf->tset->x,&cdat
                                                        ,moead_fcrbf->mdl_rbf[i].p_s
                                                        ,moead_fcrbf->kernel[i]
                                                        ,moead_fcrbf->mdl_rbf[i].m_phi);

        moead_fcrbf->mdl_rbf[i].p_m_w = mgn_rbf_new_weight(moead_fcrbf->mdl_rbf[i].m_phi
                                                        , moead_fcrbf->tset->f, 0);

        gsl_matrix *y_p = gsl_matrix_alloc(moead_fcrbf->tset->f->size1, moead_fcrbf->tset->f->size2);
        gsl_blas_dgemm(CblasNoTrans,CblasNoTrans,1
                       ,moead_fcrbf->mdl_rbf[i].m_phi
...
//        gsl_vector_fprintf(stdout,moead_fcrbf->mdl_rbf[i].p_s, "%.4f");
        moead_fcrbf->mdl_rbf[i].m_phi = mgn_rbf_create_phi(moead_fcrbf->tset->x,&cdat
                                                        ,moead_fcrbf->mdl_rbf[i].p_s
                                                        ,moead_fcrbf->kernel[i]
                                                        ,moead_fcrbf->mdl_rbf[i].m_phi);
        ...
        moead_fcrbf->mdl_rbf[i].p_m_w = mgn_rbf_new_weight(moead_fcrbf->mdl_rbf[i].m_phi
                                                        ,moead_fcrbf->tset->f,moead_fcrbf->mdl_rbf[i].p_m_w);

//        gsl_vector_fprintf(stdout,moead_fcrbf->mdl_rbf[i].p_s, "%.4f");
//        puts("_-_");

        // more DEBUG
        y_p = gsl_matrix_alloc(moead_fcrbf->tset->f->size1, moead_fcrbf->tset->f->size2);
        gsl_blas_dgemm(CblasNoTrans,CblasNoTrans,1
                       ,moead_fcrbf->mdl_rbf[i].m_phi
                       ,moead_fcrbf->mdl_rbf[i].p_m_w,0
                       ,y_p);
    ...
    gsl_vector *lambda = mgnp_moead_fcrbf_find_lambda(moead_fcrbf->tset
                                                   , moead_fcrbf->mdl_rbf, moead_fcrbf->mdl_size);
...
//    exit(0);
    ...
    gsl_matrix *m_1phi = gsl_matrix_alloc(1,moead_fcrbf->mdl_k);

    moead_fcrbf->model_data->lhci = moead_fcrbf->lhci;
    moead_fcrbf->model_data->lambda = lambda;
    moead_fcrbf->model_data->km = &cdat;
    moead_fcrbf->model_data->mphi = m_1phi;
...
mgn_pop_proto* mgn_moead_fcrbf_pop_get(mgnMoa *moa)
{
    return (mgn_pop_proto*)mgn_moead_fcrbf_features(moa)->solution;
}
...
)
{
    ...
    mgnp_moead_fcrbf_data *mrbf = malloc(sizeof(*mrbf));
    ...
    mrbf->max_run = (max_eval - nt) / W->size1;
    mrbf->nt = nt;
    mrbf->m_w = W;
    mrbf->solution = A;
...
//    mrbf->kernel = malloc(sizeof(mrbf->kernel) * mrbf->mdl_size);
    ...
    moa->stop = mgnp_moead_fcrbf_stop;
    moa->set_ga_vals = mgnp_moead_fcrbf_set_ga_vals;
    moa->features = mrbf;
    moa->pop_get = mgn_moead_fcrbf_pop_get;
...
void mgn_moa_moead_fcrbf_free(mgnMoa* moead_fcrbf)
{
    ...
    mgn_lhci_free(mrbf->lhci);
    mgnp_moead_fcrbf_mdl_mop_param_free(mrbf);
